...
async function copyText(text, button, defaultText) {
  try {
    ...
    window.prompt("게시글 주소를 복사하세요.", text);
  }
}
  ...
  fields.viewer.innerHTML = `
    <div class="guide-viewer-head" data-guide-id="${escapeHtml(post.id)}">
      ...
      <div class="guide-viewer-actions">
        <button class="guide-like-button${likedPostIds.has(post.id) ? " is-liked" : ""}" type="button" data-guide-action="like"${likedPostIds.has(post.id) ? " disabled" : ""}>${likedPostIds.has(post.id) ? "완료" : "좋아요"}</button>
        <button type="button" data-guide-action="copy-permalink">공유</button>
        ...
        <button type="button" data-guide-action="close">목록</button>
      </div>
    </div>
...
fields.viewer.addEventListener("click", (event) => {
  ...
  if (button.dataset.guideAction === "copy-permalink") copyText(guidePostHref(post.id), button, "공유");
